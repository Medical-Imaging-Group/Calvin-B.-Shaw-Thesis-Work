...
function [fwd_mesh,pj_error] = reconstruct_cw_Lp(fwd_mesh,...
						recon_basis,...
						data_fn,...
						iteration,...
						lambda,...
...
%%%%%%%%%%%%%Lpsyntehsis%%%%%%%%%%%
 foo = LpSynthesis(J, J', data_diff,2,1, 1e-6,230,1e-6,0.15);%LP Algorithm
...
foo =foo.*(recon_mesh.mua);
  ...
  %fwd_mesh.mua = fwd_mesh.mua + (foo);
   ...
   recon_mesh.mua(1) =recon_mesh.mua(2);
  jk = find(recon_mesh.mua(:) <= 0);
  recon_mesh.mua(jk) =0;
%   
%      jk = find(recon_mesh.mua(:) >= 0.025);
%    recon_mesh.mua(jk) = 0.025;
