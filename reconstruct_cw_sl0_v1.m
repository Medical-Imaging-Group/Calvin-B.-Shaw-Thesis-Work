...
function [fwd_mesh,pj_error] = reconstruct_cw_sl0(fwd_fn,...
					      recon_basis,...
					      frequency,...
					      data_fn,...
					      iteration,...
...
%   reg = eye(ncol).*lambda;%*max(diag(Hess));
...
%   J_Pseudo = J'*(Hess\eye(nrow));
  
%   H = J_Pseudo*J;%(Hess)\(J'*J);
  
  % Calculate update
%   foo = J_Pseudo*data_diff;%(Hess\(J'*data_diff));
  
  % Define algorithm parameters
%     lambda1 = 0.008;                                      % lambda : regularization parameter 0.008 --8mm
%     Nit = 50;                                           % Nit : number of iterations
%     mu1 = 0.2;                                           % mu : ADMM parameter 0.2 --8mm

    % Run BPD algorithm
%     [x_BPD, cost] = bpd_salsa_sparsemtx(foo, H, lambda1, mu1, Nit);

% foo= (double(L0Smoothing(uint8(H*100000),0.01)))/100000;

% foo = x_BPD;

     foo = (SL0_v1((J),(data_diff),0.00001));%10
     
%      lambda = 1;
% reg = diag(ones(size(A,2),1)*lambda);
% if nargin < 4
%     sigma_decrease_factor = 0.98;
% %     A_pinv = pinv(A) + reg;
%     A_pinv = (A'*A+reg)\A';
%     mu_0 = 2;
%     L = 3;
